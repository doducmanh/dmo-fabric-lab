...
# ###### _Gemini API key from internet:_
# - How to get free Gemini API Key: https://www.youtube.com/watch?v=quStyAldzFw
# - Gemini API Rate Limit: https://ai.google.dev/gemini-api/docs/rate-limits
# - Goole AI Studio: https://aistudio.google.com/prompts/new_chat
# ###### _API key from LLM Farm:_
# - LLM Farm: https://inside-docupedia.bosch.com/confluence2/genaiplatform/llm-farm-372737736.html
# - LLM Farm Community: https://teams.microsoft.com/l/team/19%3AJRlWt5L-7p8FnxdhDJIL_D_CwphjRtfx-SEHA2851fo1%40thread.tacv2/conversations?groupId=0c4f744c-7ed8-4c04-8635-ff405eae64c2&tenantId=0ae51e19-07c8-4e4b-bb6d-648ee58410f4
# - How to Order and Get Started: https://service-management.bosch.tech/sp?id=sc_cat_item&sys_id=4ba8219fc3982a503e43879dc001315a&referrer=popular_items
# - Access & Network Tipps: https://inside-docupedia.bosch.com/confluence2/genaiplatform/llm-farm-access-network-tipps-562171341.html
# - Coding Examples: https://inside-docupedia.bosch.com/confluence2/genaiplatform/llm-farm-coding-examples-562171306.html
# - Secure & Compliant: You can use the LLM Farm with data up to security class C-SC2.
# - Costs: 
#     - DEVELOPER SUBSCRIPTION: Free of charge, with limited access to the models GPT4o-mini and Gemini 2.0 Flash Lite
#     - ENTERPRISE SUBSCRIPTION: Fixed Monthly Fee + Pay-per-use
...
# Retrieve the API key's value from the broadcast variable.
api_key = broadcasted_key.value

# Define the name of the LLM model to be used.
...
from tenacity import retry, wait_exponential, wait_fixed, stop_after_attempt, retry_if_exception_type, RetryError
...
from pyspark.sql.functions import udf, col, lit, json_tuple
...
# ## <mark>1. Define function to summarize text</mark>

# CELL ********************

def summarize_text(text):
    """
    A function to summarize a given text using the Gemini API.
    Args:
    ...
    # Function to generate content from the model with a retry strategy to avoid overwhelming the API with repeated requests.
    # Only retry if a 'ResourceExhausted' exception occurs, which typically indicates that the API's rate limits have been hit.
    @retry(
...
# #### Demo: Summarize the column named `product_description` and save the output in a new column named `GenAI_summary_product_description`.
# Problem to solve: The `product_description` column contains lengthy paragraphs that need to be summarized for clarity and brevity.

# CELL ********************

# product_description before summarize
...
# product_description after summarized
...
# ## <mark>2. Define function to translate a given text to a target language</mark>
...
# #### Demo: 
# #### - Translate column `product_review` to English and save the output in a new column `GenAI_English_product_review`
# #### - Translate column `product_review` to Vietnamese and save the output in a new column `GenAI_Vietnamese_product_review`
# Problem to solve: The `product_review` column contains reviews in multiple languages and needs to be translated into a single target language.

# CELL ********************

# product_review before translate
...
# product_review after translated to English & Vietnamese
df_translate = df_translate.withColumn("GenAI_English_product_review", translate_udf('product_review', lit('english')))
df_translate = df_translate.withColumn("GenAI_Vietnamese_product_review", translate_udf('product_review', lit('vietnamese')))
display(df_translate)
...
# ## <mark>3. Define function to detect sentiment of a given text and returns a score between 0 and 1. 
# ### Scores closer to 1 are positive, closer to 0 are negative.</mark>
...
# #### Demo: After translated `product_review` to `GenAI_English_product_review`. Perform detect sentiment score for column `GenAI_English_product_review`. And save the output as `GenAI_sentiment_product_review`.

# CELL ********************

df_sentiment = df_translate.withColumn("GenAI_sentiment_product_review", sentiment_udf('GenAI_English_product_review'))
...
# ## <mark>4. Define function to corrects the spelling, grammar, and punctuation of a given text.</mark>
...
# #### Demo: correct spelling, grammar for column `product_properties`. And save the output as `GenAI_correct_product_properties`.
# Problem to solve: The `product_properties` column contains spelling and grammar errors that need to be corrected.

# CELL ********************

# column product_properties currently has spelling and grammar errors and needs to be fixed.
...
# ## <mark>5. Define function to extracts person names, locations, and organization names from a given text.</mark>
...
# #### Demo: extract person names, locations, and organization names for column `product_origin`. Save the output in column `GenAI_entities_product_origin` with json format.

# CELL ********************

# column "product_origin" before extract entities.
...
# after extracted locations, organizations, persons
df_extract_entities = df_extract_entities.withColumn("GenAI_entities_product_origin", entity_extraction_udf('product_origin'))
df_extract_entities = df_extract_entities.select(
    "product_name",
    "product_origin",
    "GenAI_entities_product_origin",
    json_tuple("GenAI_entities_product_origin", "persons", "locations", "organizations")
).toDF("product_name", "product_origin", "GenAI_entities_product_origin", "persons", "locations", "organizations")
display(df_extract_entities)
...
# ## <mark>6. Define function to assigns a category label to a given text from a provided list of labels.</mark>
...
# #### Demo: Categorize column `product_name` into categories "engineering, home appliance, automotive, power tools". Save the output in column `GenAI_labels_product_name`.

# CELL ********************

# Before categorize "product_name"
...
# After categorized "product_name"
